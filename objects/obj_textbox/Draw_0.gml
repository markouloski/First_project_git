...
if setup == false
	...
	for(var p = 0; p < page_number; p++)
		...
		for (var c=0; c < text_length[p]; c++)
		{
			...
			//store indivdual chars in the "char" array
			...
			if _current_text_w - line_break_offset[p] > line_width
			{
				...
				line_break_num[p]++;
				var _txt_up_to_last_string = string_copy( text[p], 1, last_free_space);
				var _last_free_space_string = string_char_at( text[p], last_free_space);
				line_break_offset[p] = string_width(_txt_up_to_last_string) - string_width(_last_free_space_string);
			}
		}
		...
		for (var c = 0; c < text_length[p]; c++)
		{
			...
			if _char_pos >= line_break_pos[lb, p]
			{
				...
				//record the line a character should be on
				_txt_line = lb + 1; //lb + 1 since lb starts at 0
			}
		}
